...
class MLeagueScoreService {
        ...
        try {
            ...
            DB::transaction(function () use ($rows): void {
                foreach ($rows as $name => $row) {
                    Player::where('name', $name)->update(['team_name' => $row['team'], 'season_point' => $row['point']]);
                }
    ...
    public function parse(string $html): array
    {
        ...
        foreach ($xpath->query("//*[contains(concat(' ', normalize-space(@class), ' '), ' p-stats__teamName ')]") as $heading) {
            ...
            if ($names->length !== 4 || $pointCells->length !== 4) throw new RuntimeException('成績表の列数が不正です。');
            for ($i = 0; $i < 4; $i++) {
                ...
                $rows[$name] = ['team' => $team, 'point' => number_format((float)$raw, 1, '.', '')];
            }
        }
