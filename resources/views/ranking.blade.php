...
<div class="section-head">
    <div>
        ...
        <p class="page-intro">カテゴリーごとに4選手の今季ポイントを合計し、順位と最下位との差を表示します。</p>
    </div>
...
@foreach($categories as $category)
    <section class="ranking-category">
        <div class="category-heading">
            <p class="eyebrow">CATEGORY {{ str_pad((string) $loop->iteration, 2, '0', STR_PAD_LEFT) }}</p>
            <h2>{{ $category->name }}</h2>
        </div>
        @php($previousTotal = null)
        @php($rank = 0)
        @php($highestTotal = $category->groups->first()?->players->sum(fn ($player) => (int) round((float) $player->season_point * 10)))
        @php($lowestTotal = $category->groups->last()?->players->sum(fn ($player) => (int) round((float) $player->season_point * 10)))
        ...
        @forelse($category->groups as $group)
            ...
            @php($rankChange = $group->pivot->previous_rank === null ? null : $group->pivot->previous_rank - $rank)
            <section @class(['card', 'rank-card', 'is-last' => $isLast])>
                <div class="rank-header">
                    ...
                    <div class="rank-score"><strong class="point {{ $total > 0 ? 'plus' : ($total < 0 ? 'minus' : '') }}">{{ $total > 0 ? '+' : '' }}{{ number_format($total, 1) }} pt</strong><span class="rank-gap">{{ $isLast ? '最下位との差 0.0 pt' : '最下位との差 '.number_format(($totalTenths - $lowestTotal) / 10, 1).' pt' }}</span></div>
                </div>
        ...
        @empty
            <p class="card">このカテゴリーにはグループが登録されていません。</p>
        @endforelse
        </div>
    </section>
@endforeach
